...
fn test_grade_for_one_student() {
    let mut s = school::School::new();
    s.add(2, "Aimee");
    assert_eq!(s.grade(2), some_strings(vec!["Aimee"]));
}
...
fn test_grade_returns_students_sorted_by_name() {
    ...
    s.add(2, "Blair");
    s.add(2, "Paul");
    assert_eq!(s.grade(2), some_strings(vec!["Blair", "James", "Paul"]));
}
...
fn test_add_students_to_different_grades() {
    ...
    assert_eq!(s.grade(3), some_strings(vec!["Chelsea"]));
    assert_eq!(s.grade(7), some_strings(vec!["Logan"]));
}
