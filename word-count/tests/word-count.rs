...
fn test_count_one_of_each() {
    check_word_count("one of each", vec![("one", 1), ("of", 1), ("each", 1)]);
}

#[test]
fn test_count_multiple_occurrences() {
    check_word_count("one fish two fish red fish blue fish",
                     vec![("one", 1), ("fish", 4), ("two", 1), ("red", 1), ("blue", 1)]);
}

#[test]
fn test_ignore_punctuation() {
    check_word_count("car : carpet as java : javascript!!&@$%^&",
                     vec![("car", 1),
                          ("carpet", 1),
                          ("as", 1),
                          ("java", 1),
...
fn test_include_numbers() {
    check_word_count("testing, 1, 2 testing",
                     vec![("testing", 2), ("1", 1), ("2", 1)]);
}

#[test]
fn test_normalize_case() {
    check_word_count("go Go GO Stop stop", vec![("go", 3), ("stop", 2)]);
}
